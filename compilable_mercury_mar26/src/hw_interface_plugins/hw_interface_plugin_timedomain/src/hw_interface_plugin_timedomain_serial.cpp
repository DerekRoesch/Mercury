...
void hw_interface_plugin_timedomain::timedomain_serial::rosMsgCallback(const hw_interface_plugin_timedomain::Range_Request::ConstPtr &msg)
{
    ...
    if((!isRequestInProgress() || ((ros::Time::now().toSec()-requestTime.toSec())>REQUEST_TIMEOUT) && msg->send_range_request && (msg->radio_id_to_target >= 0)))
    {
        if((ros::Time::now().toSec()-requestTime.toSec())>REQUEST_TIMEOUT)
        {
            ROS_ERROR("Radio Timed Out!");
        }
        requestTime=ros::Time::now();
